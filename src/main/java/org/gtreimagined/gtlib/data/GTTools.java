...
public class GTTools {
    ...
    public static final GTToolType BRANCH_CUTTER = GTAPI.register(GTToolType.class, new GTToolType(Ref.SHARED_ID, "branch_cutter", 1, 3, 2, 0.0F, -1.5F, false)).addTags("grafter").addEffectiveBlockTags(BlockTags.LEAVES).setHasContainer(false).setDurabilityMultiplier(0.25f);
    public static final GTToolType KNIFE = GTAPI.register(GTToolType.class, new GTToolType(Ref.SHARED_ID, "knife", 1, 2, 1, 2.1F, -2.0F, false)).addEffectiveBlocks(Blocks.COBWEB).setRepairable(false).setMaterialTypeItem(KNIFE_HEAD).setTag(new ResourceLocation(Ref.ID, "knives")).setOriginalTag(true);
